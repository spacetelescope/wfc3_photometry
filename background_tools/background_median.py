...
apertures.
    This is primarily intended for estimating backgrounds via annulus 
apertures.
    The intent is that this falls easily into other code to provide 
background measurements.
    ...
    apertures : photutils.aperture.core.PixelAperture object (or 
subclass)
...
'center' the pixels will only be
        included if the pixel's center falls within the aperture.
...
aperture_median, aperture_mode,
        and a row for each of the positions the apertures were created 
at.
    ...
    aperture_stats = np.array([calc_aperture_mmm(data, mask) for mask in 
masks])
...
falling within some
    Photutils aperture mask on some array of data.
    """
